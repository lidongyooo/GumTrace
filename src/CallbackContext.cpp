...
CALLBACK_CTX* CallbackContext::pull(const cs_insn* _instruction, csh _handle, std::string module_name, uint64_t module_base) {
    ...
    ctx->handle = _handle;
    ctx->module_name = std::move(module_name);
